...
/**
 * Copyleft (C) 2016 by Aaron Digulla. Use as you wish. This copyright notice
 * can be removed.
 */
 ...
 * "https://sites.google.com/site/baohuagu/how-to-detect-if-a-drive-is-network-drive">How
 * to detect if a drive is network drive?</a> and <a href=
 * "http://stackoverflow.com/questions/9163707/java-how-to-determine-the-type-of-drive-a-file-is-located-on">Java:
 * how to determine the type of drive a file is located on?</a>
 */
public class DangerousPathChecker {
    ...
    public boolean isDangerous(File file) {
        ...
        if (path.startsWith("//") || path.startsWith("\\\\")) {
            // We might want to check for \\localhost or \\127.0.0.1 which would
            // be OK, too
            return true;
        }
        ...
        if (!":".equals(colon)) { throw new IllegalArgumentException("Expected 'X:': " + path); }
     ...
     * <code>net use</code> will return an error for anything which isn't a
     * share.
     * 
     * <p>
     * Another option would be <code>fsinfo</code> but my gut feeling is that
     * <code>net</code> should be available and on the path on every
    ...
    public static void main(String[] args) {
        ...
        checker.check("");
        checker.check("//SCHOBBAK/Backup");
        checker.check("\\\\SCHOBBAK\\Backup");
        checker.check("C:");
        checker.check("C:/Windows");
        checker.check("C:\\Windows");
        // On my computer, S: is a mapped network drive
        checker.check("S:");
        checker.check("S:/PC-LAMBECK-02");
        checker.check("S:\\PC-LAMBECK-02");
    }
