...
public class ValidationController extends Thread {
    ...
    public ValidationController(IDataModelController dataModelController) {
        super();
        if (dataModelController == null) { throw new InvalidParameterException(
                "DataModelController must not be null"); }
        ...
        try {
            while (state != Thread.State.TERMINATED) {
                ...
                if ((model != null) && (!model.isModelChecked())) {
                    /*
                     * Set to true as early as possible: If the state will be
                     * set to "unchecked" due to changes to the model, it won't
                     * be overwritten at the end.
                     */
                    model.setModelChecked(true);
                    model.setModelValidity(false);
                    boolean isModelValid = true;
                    IValidationMessagesPanel msgPanel = dataModelController
                            .getValidationMessagePanel(model.getModelName());
